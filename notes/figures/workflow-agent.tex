...
\node[conceptlabel, anchor=south] at (-1.4,1.25) {code sets each step};
\node[conceptlabel, anchor=south] at (7.4,1.25) {the model sets each step};
% the two blocks
...
\node[conceptlabel, text width=4.4cm, anchor=north] at (6,-0.75)
  {model chooses the next step at run time\\loops until the task is done\\flexible and open-ended\\higher cost, more room for error};
\end{tikzpicture}
