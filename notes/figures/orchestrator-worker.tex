...
\node[box, fill=orange!15] (orch) at (0,0) {Orchestrator};
\node[sub, fill=blue!15] (s1) at (4.7,1.7)  {sub-agent\\\emph{own context}};
\node[sub, fill=blue!15] (s2) at (4.7,0)    {sub-agent\\\emph{own context}};
\node[sub, fill=blue!15] (s3) at (4.7,-1.7) {sub-agent\\\emph{own context}};
\node[box, fill=green!15] (syn) at (9.4,0) {Synthesis};
\draw[arr] (orch) -- (s1);
