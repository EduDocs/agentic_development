...
\node[conceptbox, fill=gray!15, minimum width=3.4cm] (pop) at (0, 2.0) {Initial population $\Population_0$};
\node[conceptbox, fill=violet!15] (gen) at (0,    0)    {Generate};
\node[conceptbox, fill=green!15]  (fit) at (5.2,  0)    {Evaluate\\(Fitness)};
\node[conceptbox, fill=orange!15] (sel) at (10.4, 0)    {Select};
