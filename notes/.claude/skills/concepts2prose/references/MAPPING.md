# concepts2prose — Concept Mapping (sidecar → shipped prose)
...
`concepts2prose` **expands**: a compact concept in the sidecar becomes one or more polished
sentences in the shipped prose. It is the asymmetric counterpart of `prose2concepts` (which
compresses).
...
ahead of the sidecar — say so and suggest `/prose2concepts`. Proceed only on confirmation.
...
Re-running `concepts2prose` on a section whose spine is already realized finds no missing/divergent
