...
The model's output is entirely determined by its input.
Two identical contexts produce identical distributions over outputs.
If the agent behaves unexpectedly, the cause is in the context — either something unexpected is present, or something necessary is absent.
\end{principle}
...
Empirical studies consistently find \emph{recency bias} (later tokens receive more weight) and \emph{primacy bias} (early tokens, particularly system-prompt content, also receive elevated attention), with a \emph{lost-in-the-middle} effect where relevant information buried in the center of a long context is underused.
...
The \emph{temperature}\index{Temperature} parameter scales the logits before sampling: a temperature of~0 produces deterministic (greedy) output; higher temperatures produce more varied output.
...
\emph{Chain-of-thought prompting}\index{Chain-of-thought prompting} encourages the model to reason through a task step by step before producing a final answer.
