...
\begin{tikzpicture}[
    node distance=1.5cm,
    step/.style={draw, rounded corners=3pt, minimum width=3.2cm,
...
\node[step, fill=yellow!25]   (t1) {Thought};
\node[step, fill=blue!20,  right=1.6cm of t1] (a1) {Action (tool call)};
\node[ctx,  below=0.7cm of a1] (ob) {Observation (tool result)};
\node[step, fill=yellow!25, below=0.7cm of ob] (t2) {Thought};
\node[step, fill=green!20,  right=1.6cm of t2] (fin) {Final Answer};
...
\begin{remark}
Modern frontier models frequently use chain-of-thought-like reasoning internally (visible as \emph{extended thinking} in some APIs) even without being explicitly prompted to reason before acting.
Whether or not this internal reasoning is visible, the tool-call sequence itself constitutes an auditable record of the agent's decision-making.
\end{remark}
