...
Note also that \code{min} is the correct selector only because the score here is an error, for which lower is better; under a fitness convention where higher is better, the same line would need \code{max}.
\end{example}
...
\begin{figure}[htb!]
\begin{center}
\input{figures/orchestrator-worker}
