...
Ch.2's concept-level rig. Collects what a student needs at the keyboard (launching, sessions,
...
- Every section is wired back to its conceptual home: sessions → Ch.2, permission modes →
  Principle LeastPrivilege + plan-then-execute as Ch.4's specify–execute–read loop,
  instruction layers → Ch.2/Ch.3, /context & /compact → Ch.3, headless flags → Ch.8
  (allowedTools = per-invocation least privilege; the permission config IS the supervision),
  worktrees → the physical substrate for Ch.8's parallel agents.
...
- Workshop framings absorbed into chapters: architecture-not-model thesis, demand paging,
  filesystem-as-scratchpad (already implicit in Ch.3/Ch.8), compressed index, verification oracle.
