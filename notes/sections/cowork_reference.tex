...
\emph{Code} --- Claude Code, and OpenAI's Codex CLI and its hosted agent counterpart --- is delegation specialized to a repository: editing files, running tests, working through git.
The Codex name has been reused across three unrelated products, so a reader chasing the reference should be specific: the original code-completion \emph{model} of 2021 is retired, and what belongs in this comparison is the agentic coding environment of 2025 onward, in its local-CLI and cloud-hosted forms.
