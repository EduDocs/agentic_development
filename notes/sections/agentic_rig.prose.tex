...
An agent in a managed sandbox (a virtual-machine shell, as in Cowork) reaches a preloaded set of general tools and can install more, but a wall seals it off from the machine's own toolchain; it touches the researcher's files only through an explicitly connected folder, reached over a permissioned channel rather than by arbitrary host access.
...
Locally (left), the agent reaches the machine's full toolchain and files; in a sandboxed virtual machine (right), it reaches general preloaded tools but is walled off from the machine's licensed, compiled, and domain-specific software, reaching the researcher's files only through an explicitly connected folder over a permissioned channel.}
...
git commit -m "Add loader for the benchmark dataset"  # record the snapshot
...
Grant the agent adequate permissions to complete its task, and that maintain the integrity of your environment.
