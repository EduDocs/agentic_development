...
The loop described in Chapter~\ref{chapter:AgenticParadigm} becomes concrete when the researcher must actually implement it.
...
Second, it exposes the agent's \emph{rationale}: the thought steps let the researcher follow the reasoning the agent gave for each tool call. Read this as a debugging aid rather than a definitive account of why the agent acted, since a model's stated reasoning need not faithfully report the computation that produced its action.
...
for step in range(MAX_STEPS):
    reply = client.messages.create(
        model=MODEL, max_tokens=4096, tools=TOOL_DEFS, messages=messages
    )
...
That workaround is becoming a fallback rather than the main mechanism.
Frontier models increasingly support \emph{interleaved} thinking: reasoning is resumed between tool calls as a native part of the loop, so the model deliberates over each returned result without a scratchpad tool to hold the turn open.
Where that is available, the thinking budget stops being a single pre-loop allowance and becomes a per-step parameter, and the no-op tool is worth adding only for models that do not interleave natively.
...
A workflow is \emph{repeatable} (absent external nondeterminism such as a clock, a network call, or unseeded randomness, the same input yields the same output), cheap (it spends no tokens per run), fast, and auditable, since the code is its own specification.
