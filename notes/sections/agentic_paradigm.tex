...
A \emph{token}\index{Token} is the unit in which the model reads and writes, roughly a common word or a fragment of one; as a rule of thumb English prose runs about four tokens for every three words (a token is roughly three quarters of a word), so a window of two hundred thousand tokens holds a few hundred pages of text.
