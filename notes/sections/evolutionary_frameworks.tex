...
\item $\Fitness : \Artifacts \to \RealNumbers$ is the fitness function (lower is better, following the course-wide sign convention of Section~\ref{subsection:MultiObjectiveScoring}),
...
The recurrence describes the loop once it is running.
Getting $\Population_0$ in the first place is a separate move: a generation pass proposes candidates from the problem statement alone, with no survivor to derive them from, which is why Figure~\ref{figure:EvolutionaryLoop} draws \emph{Generate} and \emph{Mutate} as two nodes rather than one.
Eq.~\eqref{equation:EvolutionaryLoop} governs every step after that seeding.
...
\begin{figure}[htb!]
\begin{center}
\input{figures/evolutionary-loop}
...
The symbolic-regression vehicle makes fitness look like a formula because its outputs are numbers.
Many discovery problems are not so lucky: an evolving experimental design, proof sketch, or protocol has no mean squared error, and a researcher reading this chapter with such a problem in hand needs to know the loop still applies.
It does, because the evaluability spectrum of Section~\ref{section:LLMJudge} already covers this case.
A rubric-driven critic model scoring each candidate is a fitness function in the sense of Definition~\ref{definition:EvolutionaryLoop}: it is automatic, it returns a number, and it can be called ten thousand times.
What it is not is objective, and the Goodhart pressure of this section applies to it with full force and one extra edge.
A weak judge is exploited exactly as a weak metric is, but its holes are rhetorical rather than numerical: the search learns which phrasings, hedges, and formatting the judge rewards, and converges on candidates that read well to that judge rather than ones that are better.
The countermeasures are the ones Section~\ref{section:LLMJudge} names --- pin the judge model and rubric before the run, calibrate against a small human-scored set, prefer relative judgments to absolute scores --- plus one the loop makes necessary: audit a sample of scored candidates by hand every so many generations, because nothing else in an unattended run will notice the drift while it is happening.
...
Common exploits:
\begin{enumerate}
\item A fitted constant, the cheapest expression the space contains at complexity~1 under the node-count rule of Section~\ref{subsection:Parsimony}.
If $\mathrm{MSE}$ is computed on the validation split, a global constant performs worse than a simple model on diverse points; but if $\lambda$ is too large relative to the MSE gap, the complexity saving can still dominate.
...
It is the most useful thing to read next: every component of Definition~\ref{definition:EvolutionaryLoop} appears in it under a recognizable name, so a researcher who has followed this chapter can open the repository and trace mutation operator, fitness function, and population management onto the tuple rather than reverse-engineering an unfamiliar architecture.
...
A \emph{baseline ladder}\index{Baseline ladder} is a sequence of methods of increasing sophistication against which a discovery system is compared.
Beating every rung is not required; being comparable to it, on a well-designed harness, is.
The ladder matters most at its bottom: a system that cannot beat linear regression on a dataset is not a discovery system yet, however sophisticated its machinery, and the cheapest way to find that out is to run the cheap rung first.

\begin{table}[htb!]
\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Rung} & \textbf{Method} & \textbf{Where it is built} \\
\midrule
1 & Linear regression & standard tooling; no agent \\
2 & Proposer pipeline (model proposals) & Ch.~\ref{chapter:AgentLoop} \\
2b & PySR (classical GP-based SR) & external library; no agent \\
3 & Retrieval-grounded pipeline & Ch.~\ref{chapter:Retrieval} \\
4 & Orchestrated multi-proposer & Ch.~\ref{chapter:Orchestration} \\
5 & Evolutionary framework & this chapter \\
\bottomrule
\end{tabular}
\caption{The baseline ladder.
Each rung is a more capable method than the one below it, and the third column names where in these notes it is assembled; rungs 1 and 2b are conventional methods that use no agent at all, which is what makes them honest comparators.
All rungs are scored by the same harness on the same dataset, so a stronger method is expected to score lower (better) than a weaker one.}
...
\item OpenEvolve repository — open-source AlphaEvolve implementation; the hackable reference codebase in which the components of Definition~\ref{definition:EvolutionaryLoop} can be read off directly.
