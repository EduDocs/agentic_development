...
\begin{tikzpicture}
\node[conceptbox, fill=yellow!20, minimum width=3.4cm] (pop) at (0, 2.4)   {Initial population $\Population_0$};
\node[conceptbox, fill=blue!8]          (gen) at (0, 0.8)   {Generate};
\node[conceptbox, fill=blue!15]         (mut) at (0,-0.8)   {Mutate};
\node[conceptbox, fill=green!20]        (fit) at (4.4, 0)   {Evaluate\\(Fitness)};
\node[conceptbox, fill=orange!20]       (sel) at (4.4,-2.2) {Select};

\draw[conceptflow] (pop) -- node[conceptlabel,right]{seed} (gen);
\draw[conceptflow] (gen.east) -- node[conceptlabel,above,pos=0.55]{initial}    (fit);
\draw[conceptflow] (mut.east) -- node[conceptlabel,below,pos=0.55]{candidates} (fit);
\draw[conceptflow] (fit) -- node[conceptlabel,right]{scored} (sel);
\draw[conceptflow] (sel.south) -- ++(0,-0.8) -| node[conceptlabel,pos=0.25,below]{survivors $\Population_{t+1}$} (mut.south);
\end{tikzpicture}
