...
\caption{This figure illustrates a Transformer, schematically, following Vaswani et al.\ (2017).
An input embedding is combined with a \emph{positional encoding} and passed through a stack of decoder blocks (repeated $n$ times).
Each block applies masked self-attention, then encoder-decoder attention, then a position-wise feed-forward layer, with every sublayer wrapped in a residual connection and layer normalization (add \& normalize).
A final linear projection and softmax turn the top-layer representation into a conditional probability distribution over the next token.
The reader need not follow every box; the point is that the whole model is built from attention and feed-forward layers, and its output is a probability distribution.}
