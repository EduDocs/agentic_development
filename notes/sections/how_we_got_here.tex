\chapter{How We Got Here: From Word Vectors to Agents}
...
They did not.
The capability that makes an agent possible --- a single model that reads instructions, holds a conversation, writes code, and calls tools --- is the accumulated payoff of roughly a decade of specific, dated advances, each of which removed a concrete limitation of the one before it.
This chapter traces that lineage at a high level.
...
The dates are close together --- the span from the first milestone to working agents is little more than a decade --- which is part of why the current moment feels sudden even though the path is legible in hindsight.

\section{Meaning as Geometry (2013--2014)}
\label{section:MeaningAsGeometry}

The first step was to give words a form a machine could compute with.
...
The decisive architectural step replaced recurrence with \emph{attention}.
The \emph{Transformer}\index{Transformer} let every position in a sequence attend directly to every other, dispensing with the sequential bottleneck and --- because attention is parallel where recurrence is serial --- making it practical to train much larger models on much more data (Vaswani et al., 2017).
Every large language model the course uses is a Transformer.
Its attention mechanism is also why context has a cost: attention forms pairwise relationships across tokens, so a finite attention budget is spread more thinly as the input grows --- the effect developed in Chapter~\ref{chapter:ContextEngineering}.
...
Contextual embeddings from this family are what modern retrieval actually runs on, the successor to the static word vectors of the previous section.
...
\section{Steerable by Text: Scale, Prompting, and Grounding (2020)}
\label{section:ScaleAndGrounding}

Scaling the decoder branch produced a qualitative change, not merely a quantitative one.
\emph{GPT-3} was a 175-billion-parameter model that, with no task-specific training, could perform a new task from a plain-language instruction and a handful of examples placed in its context --- \emph{in-context}, or \emph{few-shot}, learning\index{In-context learning} (Brown et al., 2020).
...
This made knowledge updatable and attributable, and it is the direct basis of the grounding developed in Chapter~\ref{chapter:Retrieval}.
...
At that point the model is no longer only answering; it is perceiving, reasoning, and acting --- the definition of an agent that opens Chapter~\ref{chapter:AgenticParadigm}.
...
The practical consequences --- a latent reasoning trace that cannot be read as an audit log, and a thinking budget in place of a prompt scaffold --- are treated in Chapter~\ref{chapter:AgentLoop}.
...
Table~\ref{table:AgenticTimeline} lays out the sequence in one view.
...
\midrule
2013  & Word embeddings (word2vec)        & meaning as a learned vector geometry \\
...
2018  & Pre-training (BERT; the GPT line) & transferable, contextual representations \\
2020  & Scale and few-shot (GPT-3)        & steering a frozen model by text alone \\
2020  & Retrieval-augmented generation    & grounding in external, updatable knowledge \\
2022--23 & Instruction tuning, chain-of-thought, tools & following instructions and acting --- agents \\
2024--26 & Reasoning models, multimodality   & trained latent reasoning; the chain continues \\
\bottomrule
\end{tabular}
\end{table}
...
But the sequence is not arbitrary: today's agents are where this particular chain of problems and solutions has led, and the same pressure that drove it --- extend what the system can reliably do --- is what drives the rest of this book.
With the timeline in place, the next chapter turns from how we got here to what an agent is, and why general-purpose tool-using models make this the moment the pieces combine.
...
\item Vaswani, A., et al., ``Attention Is All You Need,'' \emph{NeurIPS}, 2017 — the Transformer; the architecture underlying every model the course uses.
