...
- 2026-08-09 (panel attempt-21, clarity + pedagogy, both reviewers independently): the chapter map **ended at Ch.~EvolutionaryFrameworks**, so the two appendices were discoverable only by scanning the table of contents or stumbling on the one pointer in Ch.~AgenticRig. Added a short closing paragraph naming both and, more usefully, saying *how they differ from the chapters*: they are consulted, not read in order (Appendix~RigReference as a working reference for one concrete rig, Appendix~CoworkReference for the case where the loop arrives as a product). A map that silently omits a fifth of the book is worse than no map.
