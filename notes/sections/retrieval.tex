...
The reference corpus holds domain units, functional forms, and dimensional constraints that dictates which model proposals are physically plausible.
...
The property is statistical rather than universal: typically, finite-dimensional embedding cannot order \emph{every} triple correctly; retrieval quality depends on how often the inequality holds for the passages that matter.
\end{definition}
...
\begin{tikzpicture}[
    ...
    store/.style={conceptbox, fill=gray!8,
                  minimum width=3.0cm, minimum height=1.0cm},
    arr/.style={conceptflow}
]
\node[box, fill=blue!15]                   (q)     {query $q$};
\node[box, fill=blue!8,    below=of q]      (qe)    {embed\\$\phi(q)$};
...
\node[store, above=of idx]            (corpus) {corpus $\mathcal{K}$ (sealed)\\chunk $+$ embed};
...
The corpus $\mathcal{K}$ is chunked and embedded once, offline, and sealed thereafter; exactly the discipline of Section~\ref{section:VectorSearch}.
...
\begin{remark}
Grounding helps when the retrieved prior is aligned and hurts when it is misaligned, and which case is better cannot be assumed in advance.
