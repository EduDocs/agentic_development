...
For larger corpora, \emph{approximate nearest-neighbor} (ANN) indices trade a small reduction in recall for a large reduction in query time.
...
A \emph{retrieval-augmented} agent extends the basic loop with a retrieval step before each model invocation.\index{RAG architecture}
...
\begin{example}
A dataset generated from the formula $y = e^{-x_1^2} \sin(x_2)$ is unusual.
...
The ablation study required in Challenge~3 — grounded vs. ungrounded on at least two datasets — exists to quantify exactly this tradeoff.
