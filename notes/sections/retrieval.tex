...
\emph{Retrieval-augmented generation} (RAG)\index{Retrieval-augmented generation} solves this by injecting relevant domain knowledge into the context at the time it is needed.
...
An \emph{embedding model} is a function $\phi: V^* \to \RealNumbers^d$ such that, for passages $p_1, p_2, p_3$ with $p_1$ more semantically similar to $p_2$ than to $p_3$,
\begin{equation}
...
print(cosine_similarity(emb_drag, emb_density))  # ~0.23 — unrelated
\end{lstlisting}
\end{example}
