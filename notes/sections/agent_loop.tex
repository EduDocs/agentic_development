...
\begin{lstlisting}[language=json]
{
  ...
  "description": "Fit a candidate symbolic expression to the training
    data and return the mean squared error on the held-out test set.
...
Frontier \emph{reasoning models}\index{Reasoning model} --- Claude's extended thinking (Anthropic, 2025) and the o3-class models --- move this step inward, reasoning internally before emitting a tool call, so the thought becomes latent rather than a visible transcript entry.
...
\item explicit checks in the loop for known failure conditions (is the test set non-empty?), and
...
\begin{small}
\begin{enumerate}
\item Yao, S., et al., ``ReAct: Synergizing Reasoning and Acting in Language Models,'' \emph{ICLR}, 2023 — the original formulation of the thought–action–observation pattern.
...
\item Shinn, N., et al., ``Reflexion: Language Agents with Verbal Reinforcement Learning,'' \emph{NeurIPS}, 2023 — on agents that detect and recover from their own failures.
