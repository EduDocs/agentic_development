...
In a ReAct agent, the model alternates between \emph{thought} steps (natural-language reasoning about what to do next) and \emph{action} steps (tool calls or final responses).
...
Since the tool-calling layer validates arguments against the schema, malformed outputs are automatically rejected and the model is asked to try again.
...
The first mechanism is preferable when the API supports it, because validation happens before the model commits to an output.
